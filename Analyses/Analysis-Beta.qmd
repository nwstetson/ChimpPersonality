...
CodedChimp6 <- rename(CodedChimp6, Reckless_R = Reckless, Implusive_R = Impulsive, Solitary_R = Solitary, Independent_R = Independent, Stable_R = Stable, 'Dependent/Follower_R' = 'Dependent/Follower')
...
```{r}
extroversion <- Trait %>% select(Sociable, Solitary_R, Active, Independent_R)
# these are the smaller personality traits that the Gombe paper put into the openness category
...
```{r}
neuroticism <- Trait %>% select(Stable_R, Excitable)
# these are the smaller personality traits that the Gombe paper put into the openness category
...
```{r}
dominance <- Trait %>% select(Dominant, 'Dependent/Follower_R', Decisive)
# these are the smaller personality traits that the Gombe paper put into the openness category
